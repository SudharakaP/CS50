...
{
    // TODO
    // uses binary search to search for input string
    
    int begin = 0, end = WORDS - 1, n = strlen(s);
    ...
    {    
        ...
        while (i <= n)
        {    
            if (dictionary.words[middle].letters[i] < s[i])
            {
                begin = middle + 1;
                break;
            }
            else if (dictionary.words[middle].letters[i] > s[i])
            {
                end = middle - 1;
                break;
            }
        ...
        if (i == n + 1 && dictionary.words[middle].found != true)
        {
...
void scramble(void)
{
    ...
    // creates a temp. grid
    int gridtemp[DIMENSION][DIMENSION];
    
    // scrambles and saves in temp. grid   
    for (int row = 0; row < DIMENSION; row++)
        for (int col = 0; col < DIMENSION; col++)
            gridtemp[col][DIMENSION - 1 - row] = grid[row][col];
            
    // transfers back to original grid            
    for (int row = 0; row < DIMENSION; row++)
        for (int col = 0; col < DIMENSION; col++)
            grid[row][col] = gridtemp[row][col];
}
